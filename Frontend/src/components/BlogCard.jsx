...
function BlogCard({blog}) {
  ...
  return (
    <div onClick={()=>navigate(`/blog/${_id}`)} className='w-full rounded-lg overflow-hidden shadow hover:scale-102 hover:shadow-primary/25 duration-300 cursor-pointer'>
        ...
        <span className='ml-5 mt-4 px-3 py-1 inline-block bg-gray-900/85 rounded-full text-(--primary-color) text-xs'>{category}</span>
        <div className='p-5'>
            <h5 className='mb-2 font-medium text-(--secondary-color)'>{title}</h5>
